...
export const ArtifactNode = memo(({ data }: any) => {
  return (
    ...
    >
      <AgentBadge agentId={data.agent_id} />
      ...
      <div className="p-4">
        <h3 className="font-bold text-gray-900 mb-2">{data.title}</h3>
...
export const BrowserNode = memo(({ data }: any) => {
  return (
    <motion.div 
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
    ...
    >
      ...
      <div className="bg-zinc-800 px-4 py-2 flex items-center gap-2 text-white">
        ...
        <div className="bg-zinc-700 rounded text-xs px-2 py-1 flex-1 font-mono text-center">
          {data.title || "active-mirror.net/search"}
        </div>
      </div>
      <div className="p-6 bg-gray-50 min-h-[200px]">
        <h3 className="font-bold text-xl text-gray-900 mb-4">{data.title}</h3>
        <div className="text-sm text-gray-700 space-y-4 whitespace-pre-wrap">{data.body}</div>
      </div>
    </motion.div>
  );
});

export const GovernanceNode = memo(({ data }: any) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h3 className="font-bold text-gray-900 mb-2">{data.title}</h3>
        <p className="text-sm text-gray-700 leading-relaxed font-mono">{data.body}</p>
      </div>
    </motion.div>
  );
});

export const ChartNode = memo(({ data }: any) => {
  return (
    <motion.div 
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
